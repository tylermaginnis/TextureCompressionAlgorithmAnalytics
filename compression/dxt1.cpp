...
void rgbFrom565(uint16_t color, uint8_t& r, uint8_t& g, uint8_t& b) {
    r = (color >> 11) & 0x1F;
    g = (color >> 5) & 0x3F;
    b = color & 0x1F;
...
void compressWithDXT1(const std::vector<uint8_t>& textureData, int width, int height, std::vector<uint8_t>& compressedData) {
    ...
    // Note about DXT1 compression
    std::cout << "DXT1 compression has a fixed compression ratio of 1:8." << std::endl;
    std::cout << "Each 4x4 pixel block is compressed to 64 bits from 512 bits." << std::endl;
    std::cout << "This results in quality loss depending on the original texture." << std::endl;

    // DXT1 compression logic
    // 1. Split the texture into 4x4 pixel blocks
    for (int y = 0; y < height; y += 4) {
        for (int x = 0; x < width; x += 4) {
            // Extract 4x4 block (16 pixels, 4 bytes per pixel)
            ...
            for (int j = 0; j < 4; ++j) {
                for (int i = 0; i < 4; ++i) {
                    ...
                    if (srcX < width && srcY < height) {
                        ...
                        if (index + 3 < textureData.size()) { // Ensure index is within bounds
            ...
            // 2. Determine a 4-color palette
            // For simplicity, we'll use the min and max colors in the block as the two extremes
            ...
            // 3. Interpolate the other two colors in the palette
            ...
            // 4. Assign each pixel an index into the palette
            ...
            for (size_t j = 0; j < block.size(); j += 4) {
                ...
                // Assign the index based on the smallest distance
            ...
            // 5. Store the palette and indices
            uint32_t packedIndices = 0;
            for (size_t k = 0; k < indices.size(); ++k) {
                packedIndices |= (indices[k] & 0x03) << (2 * k); // Ensure only 2 bits are used per index
            }
